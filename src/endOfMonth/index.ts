...
export function endOfMonth(date: AnyTemporalDate): AnyTemporalDate {
  if (date instanceof Temporal.PlainDate) return date.with({ day: date.daysInMonth });
  return date.with({ day: date.daysInMonth, hour: 23, minute: 59, second: 59, millisecond: 999, microsecond: 999, nanosecond: 999 });
}
