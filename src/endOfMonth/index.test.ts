...
describe("endOfMonth", (it) => {
    it("sets day to last day for PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        const result = endOfMonth(date);
        expect(result.toString()).toBe("2024-06-30");
    });

    it("handles February in a leap year", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-02-10");
        const result = endOfMonth(date);
        expect(result.toString()).toBe("2024-02-29");
    });

    it("sets day and time to end of month for PlainDateTime", ({ expect }) => {
    ...
    it("sets day and time to end of month for ZonedDateTime", ({ expect }) => {
