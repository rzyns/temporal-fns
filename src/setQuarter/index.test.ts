...
describe("setQuarter", (it) => {
    ...
    it("returns same date when already in the target quarter", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-05-15");
        const result = setQuarter(date, 2);
        expect(result.toString()).toBe("2024-05-15");
    });

    it("handles end-of-month clamping when moving quarters", ({ expect }) => {
