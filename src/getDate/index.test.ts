...
describe("getDate", (it) => {
    it("returns the day of the month of a PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        expect(getDate(date)).toBe(15);
    });

    it("returns the day of the month of a PlainDateTime", ({ expect }) => {
        const dt = Temporal.PlainDateTime.from("2024-01-31T10:30:00");
        expect(getDate(dt)).toBe(31);
    });

    it("returns the day of the month of a ZonedDateTime", ({ expect }) => {
    ...
    it("returns 1 for the first day of the month", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-03-01");
        expect(getDate(date)).toBe(1);
    });

    it("returns 29 for Feb 29 in a leap year", ({ expect }) => {
