...
describe("setDay", (it) => {
    ...
    it("sets PlainDate to Friday in the same week", ({ expect }) => {
    ...
    it("sets PlainDateTime to Tuesday and preserves time", ({ expect }) => {
    ...
    it("works with weekStartsOn=7 (Sunday)", ({ expect }) => {
    ...
    it("returns same day when already on the target weekday", ({ expect }) => {
        // 2024-06-12 is Wednesday (dayOfWeek=3)
        const date = Temporal.PlainDate.from("2024-06-12");
