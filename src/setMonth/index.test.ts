...
describe("setMonth", (it) => {
    it("sets the month of a PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        const result = setMonth(date, 1);
        expect(result.toString()).toBe("2024-01-15");
    });

    it("sets the month of a PlainDateTime", ({ expect }) => {
    ...
    it("clamps day when setting to Feb in non-leap year", ({ expect }) => {
