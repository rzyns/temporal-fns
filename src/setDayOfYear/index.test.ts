...
describe("setDayOfYear", (it) => {
    it("sets day of year on a PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        const result = setDayOfYear(date, 1);
        expect(result.toString()).toBe("2024-01-01");
    });

    it("sets day of year to the last day of a leap year", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        const result = setDayOfYear(date, 366);
        expect(result.toString()).toBe("2024-12-31");
    });

    it("sets day of year to Feb 29 in a leap year (day 60)", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
    ...
    it("sets day of year to the last day of a non-leap year", ({ expect }) => {
