...
describe("getDecade", (it) => {
    it("returns the decade of a PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-06-15");
        expect(getDecade(date)).toBe(2020);
    });

    it("returns the decade of a PlainDateTime", ({ expect }) => {
        const dt = Temporal.PlainDateTime.from("1999-12-31T23:59:59");
        expect(getDecade(dt)).toBe(1990);
    });

    it("returns the decade of a ZonedDateTime", ({ expect }) => {
    ...
    it("returns the decade for the start of a decade", ({ expect }) => {
        const date = Temporal.PlainDate.from("2020-01-01");
        expect(getDecade(date)).toBe(2020);
    });

    it("returns the decade for the end of a decade", ({ expect }) => {
