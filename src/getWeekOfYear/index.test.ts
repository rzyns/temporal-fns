...
describe("getWeekOfYear", (it) => {
    it("returns the ISO week number of a PlainDate", ({ expect }) => {
        // 2024-01-01 is a Monday, ISO week 1
        const date = Temporal.PlainDate.from("2024-01-01");
        expect(getWeekOfYear(date)).toBe(1);
    });

    it("returns the ISO week number of a PlainDateTime", ({ expect }) => {
        const dt = Temporal.PlainDateTime.from("2024-06-15T10:30:00");
        expect(getWeekOfYear(dt)).toBe(24);
    });

    it("returns the ISO week number of a ZonedDateTime", ({ expect }) => {
    ...
    it("returns week 1 for the first Monday of 2024", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-01-01");
        expect(getWeekOfYear(date)).toBe(1);
    });
});
