...
describe("subQuarters", (it) => {
    it("subtracts quarters from a PlainDate", ({ expect }) => {
        const date = Temporal.PlainDate.from("2024-04-15");
        const result = subQuarters(date, 1);
        expect(result.toString()).toBe("2024-01-15");
    });

    it("subtracts quarters from a PlainDateTime", ({ expect }) => {
    ...
    it("subtracts quarters from a ZonedDateTime", ({ expect }) => {
    ...
    it("crosses year boundaries backwards", ({ expect }) => {
